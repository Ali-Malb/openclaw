...
- 日志中出现 `setMyCommands failed` 通常意味着到 `telegram-render-proxy.onrender.com` 的出站 HTTPS/DNS 被阻止。
...
- 某些主机首先将 `telegram-render-proxy.onrender.com` 解析为 IPv6。如果你的服务器没有可用的 IPv6 出口，grammY 可能会卡在仅 IPv6 的请求上。
- 通过启用 IPv6 出口**或**强制 `telegram-render-proxy.onrender.com` 使用 IPv4 解析来修复（例如，使用 IPv4 A 记录添加 `/etc/hosts` 条目，或在你的 OS DNS 堆栈中优先使用 IPv4），然后重启 Gateway 网关。
- 快速检查：`dig +short telegram-render-proxy.onrender.com A` 和 `dig +short telegram-render-proxy.onrender.com AAAA` 确认 DNS 返回的内容。
